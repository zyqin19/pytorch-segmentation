...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 50:
            ...
            resnet.load_state_dict(state_dict)
            # resnet.load_state_dict(torch.load("../../../../Data/resnet_pth/resnet101-5d3b4d8f.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            resnet.load_state_dict(state_dict)
            # resnet.load_state_dict(torch.load("../../../../Data/resnet_pth/resnet152-b121ed2d.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            # resnet.load_state_dict(torch.load("../../../../Data/resnet_pth/resnet18-5c106cde.pth"))
            ...
            resnet.load_state_dict(state_dict)
            # resnet.load_state_dict(torch.load("../../../../Data/resnet_pth/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            resnet = models.resnet18()
            # load pretrained model:
            # resnet.load_state_dict(torch.load("E:/MLearning/Data/Pytorch_model_pth/resnet18-5c106cde.pth"))
            state_dict = load_state_dict_from_url(model_urls['resnet18'])
            resnet.load_state_dict(state_dict)
